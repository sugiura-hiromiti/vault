...
ここから３週間ほど無意味にコードを眺める・ふて寝するなどしましたが一向に進展せず、奥の手を使うことにしました
...
自分は普段cliで開発しています
というよりneovimを使うからcliにしている節はありますが...
そんなターミナル作業を強力に補助してくれる革命的なツールが[Amazon Q](https://github.com/aws/amazon-q-developer-cli)というアプリです
元々はfigという名前でしたがAWSに買収された際に名称がCodeWhisperに、その後なんやかんやあって今はAmazon Qという名前だそうです
このアプリにはcli入力の補完ツールという側面があります
パスが通っているコマンドやコマンドのオプションを説明付きで補完ウィンドウに表示してくれます
これが本当に便利で、正直こいつ無しでターミナル操作をするのは苦痛に感じます
更に、何故かneovimのterminalバッファも認識してちゃんと動いてくれる優れものです

Amazon Qには補完機能の他にAI機能もあります
話題のClaude Codeの様にターミナル上で動作する
